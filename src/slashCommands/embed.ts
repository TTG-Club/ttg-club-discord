import {
  ColorResolvable, EmbedBuilder, SlashCommandBuilder, TextChannel
} from 'discord.js';
...
const command: SlashCommand = {
  command: new SlashCommandBuilder()
    ...
    .addStringOption(option => {
      return option.setName('title').setDescription('Title of the embed message')
        .setRequired(true);
    })
    .addStringOption(option => {
      return option.setName('description').setDescription('Description of the embed message.')
        .setRequired(true);
    })
  ...
  execute: async interaction => {
    try {
      ...
      if (!interaction.options) return await interaction.editReply({ content: 'Something went wrong...' });
      ...
      let selectedTextChannel = interaction.channel?.client.channels.cache
        .get(options.channel.toString()) as TextChannel;

      await selectedTextChannel.send({ embeds: [embed]});

      return await interaction.editReply({ content: 'Embed message successfully sent.' });
    } catch (error) {
