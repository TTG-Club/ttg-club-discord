...
const {
  getDescriptionEmbeds, getPagination
} = useMarkdown();
...
const commandRace: SlashCommand = {
  ...
  autocomplete: async interaction => {
    try {
      ...
      const races: TRaceLink[] = _.cloneDeep(resp.data);

      await interaction.respond(races.map((race: TRaceLink) => ({
        name: race.name.rus,
        value: race.url
      })));
    } catch (err) {
  ...
  execute: async interaction => {
    try {
      ...
      const race: TRaceItem = _.cloneDeep(resp.data);

      const title = `${ race.name.rus } [${ race.name.eng }]`;
      const raceUrl = `${ API_URL }${ url }`;
      const footer = `TTG Club | ${ race.source.name } ${ race.source.page || '' }`.trim();
      ...
      embeds.main
        ...
        .addFields({
          name: 'Скорость',
          value: race.speed
            .map(speed => (`${ speed.name ? `${ speed.name } ` : '' }${ speed.value } фт.`))
            .join(', '),
          inline: true
        })
      ...
      const skills = getHTMLArrayFromPairs(race.skills.map(skill => ({
        name: skill.name,
        value: skill.description
      })));

      embeds.desc = getDescriptionEmbeds(skills)
        .map(str => (
          new EmbedBuilder()
            .setDescription(str)
        ));

      embeds.desc.push(...getDescriptionEmbeds(race.description)
        .map(str => (
          new EmbedBuilder()
            .setTitle('Описание')
            .setDescription(str)
